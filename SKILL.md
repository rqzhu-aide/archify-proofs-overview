...
When a browser is available, inspect a long statement, several prerequisites, search/selection, and any cycle explanation using [browser-check.md](references/browser-check.md). Use existing browser tools; do not build a browser driver or install an automation framework for a paper. After one failed attempt and one reasonable fallback, disclose untested behavior and stop unless browser repair was requested. Static checks and screenshots alone do not establish interaction correctness.
